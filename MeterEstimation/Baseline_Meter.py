...
class MeterObservationModel(ObservationModel):
    def __init__(
        ...
        super().__init__()
        self.states = states
        # observation 2 = multiple onsets present, 1 = note onset present, 0 = nothing present
        self.probabilities = np.ones((3, states)) / 100
        self.probabilities[0, :] = 0.99
        for idx in subbeat_idx:
            self.probabilities[:, idx] = [0.5, 0.4, 0.1]
        for idx in beat_idx:
            self.probabilities[:, idx] = [0.3, 0.5, 0.2]
        for idx in downbeat_idx:
            self.probabilities[:, idx] = [0.1, 0.6, 0.3]
    ...
    for ts_num in beats_per_measure:
        for sbpb in subbeats_per_beat:
            for tempo in tempi:
                ...
                #deleteme.append((observations, path, observation_model))

    likelihoods = np.array(likelihoods)

    best_index = likelihoods[:, 3].argmax()
    best_result = likelihoods[best_index]
    #best_data = deleteme[best_index]

    #fig, ax = plt.subplots(nrows=2, sharex=True)
    #ax[0].plot(best_data[0][:1000] * 100)
    #ax[1].plot(best_data[2].get_beat_states(best_data[1])[:1000] * 55)
    #plt.show()
    #exit(0)
...
def process_file(
    ...
    predicted_meter, predicted_tempo = estimate_meter(filename=mfn, value_aggregation="num_notes")
